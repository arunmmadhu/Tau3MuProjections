...
using std::ofstream;
using ThreeTString = TString[3];// for three subcategories of an analysis

typedef void (*InitializeLabelFunctions) (ThreeTString&, ThreeTString&, ThreeTString&);// typedef for functions that initialize labels. Can be called using an integer

//void Initialize_Labels(ThreeTString&, ThreeTString&, ThreeTString&, ThreeTString&);

void Initialize_Labels_W(ThreeTString& cat_base, ThreeTString& cat_label, ThreeTString& print_label){
...
void Initialize_Labels_ZTT(ThreeTString& cat_base, ThreeTString& cat_label, ThreeTString& print_label){
...
void makeYield_fromBDTFit_Combine_all () //for W and ZTT, maybe HF
{
    ...
    int Which_Channel = 0; // 0 = W, 1 = ZTT, 2 = HF
    
    
    InitializeLabelFunctions Initialize_Labels[] = 
    {
        Initialize_Labels_W, 
        Initialize_Labels_ZTT
    };
    
    Initialize_Labels[Which_Channel](cat_base, cat_label, print_label);
    
    
    cout<<cat_label[0]<<endl;
    ...
    //Triplet Mass Fits: fit the triplet mass for MC
    ...
    for(int i=0; i<3; i++){
      ...
      cout<<"Calculated min: "<< signal_peak_region_min[i] <<"Calculated max: "<< signal_peak_region_max[i] <<endl;
