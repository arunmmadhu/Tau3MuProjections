...
file_fam = open("W_Lumi_Limit.txt", "r")
# using the for loop
lum_tau = []
lum_mu = []
lum_e = []
...
for lu_no in [0]: #for extreme luminosities
    ...
    command_h_mu_e = "combineCards.py "+combined_card_name[0]+"_a.txt "+combined_card_name[1]+"_a.txt "+combined_card_name[2]+"_a.txt > W_Combined.txt"
    ...
    if(Whether_Hybrid):
            ...
            command_run = "combine -M AsymptoticLimits W_Combined.txt --cl 0.9 -t -1  > out_asym.txt"
            ...
            command_run = "combine -M HybridNew W_Combined.txt --cl 0.9 -t -1 --rMin "+"{:.3f}".format(rvals_min[0])+" --rMax "+"{:.3f}".format(rvals_min[0])+" --rRelAcc=0.01 --expectedFromGrid=0.025  > out1.txt"
            os.system(command_run)
            print("Running Sigma -1")
            command_run = "combine -M HybridNew W_Combined.txt --cl 0.9 -t -1 --rMin "+"{:.3f}".format(rvals_min[1])+" --rMax "+"{:.3f}".format(rvals_min[1])+" --rRelAcc=0.01 --expectedFromGrid=0.16  > out2.txt"
            os.system(command_run)
            print("Running Sigma Median")
            command_run = "combine -M HybridNew W_Combined.txt --cl 0.9 -t -1 --rMin "+"{:.3f}".format(rvals_min[2])+" --rMax "+"{:.3f}".format(rvals_min[2])+" --rRelAcc=0.01 --expectedFromGrid=0.5  > out3.txt"
            os.system(command_run)
            print("Running Sigma +1")
            command_run = "combine -M HybridNew W_Combined.txt --cl 0.9 -t -1 --rMin "+"{:.3f}".format(rvals_min[3])+" --rMax "+"{:.3f}".format(rvals_min[3])+" --rRelAcc=0.01 --expectedFromGrid=0.84  > out4.txt"
            os.system(command_run)
            print("Running Sigma +2")
            command_run = "combine -M HybridNew W_Combined.txt --cl 0.9 -t -1 --rMin "+"{:.3f}".format(rvals_min[4])+" --rMax "+"{:.3f}".format(rvals_min[4])+" --rRelAcc=0.01 --expectedFromGrid=0.975  > out5.txt"
            os.system(command_run)
            ...
            command_run = "combine -M AsymptoticLimits W_Combined.txt --cl 0.9 -t -1  > out1.txt"
            os.system(command_run)
