        ...
        def MakeLumiScanCards(self,lumi,categ,analyzed_lumi):
                ...
                for point in bdt_points:  # For loop for bdt cuts in range [X_min;X_max]
                    ...
                    for lu_no in range(len(lumi)):
                        ...
                        combined_norm_bkg = ROOT.RooProduct("combined_norm", "combined_norm", ROOT.RooArgList(exp_fact_var, bdt_norm_var, lumi_ratio))
                        bdt_integral_bkg = self.BDT_distribution.createIntegral(ROOT.RooArgSet(self.bdt), ROOT.RooArgSet(self.bdt), "Integral_Range")
                        ...
                        print( "bdt point: ", point," sig_est: ", sig_est, " bkg_est: ", bkg_est, " bkg_est_var_withNorm: ", bkg_est_var_withNorm.getVal(), " bkg_est_var_withNorm error: ", bkg_est_var_withNorm.getPropagatedError(self.results_pdf) )
...
def ReadAndCopyMinimumBDTCard_usingUL(lumi,categories,Whether_Hybrid,bdt_points):
    ...
    for cat in range(Cat_No):
            ...
            output_dir = f"limit_scans_by_lumi_W_UL/{categories[cat]}"
            os.makedirs(output_dir, exist_ok=True)  
            ...
            for lumi_index, row in enumerate(transposed):
                ...
                #Vary the frame ymax from 30 to 5 as lumi goes from 59.83 to 3000
                frame_max = 7.0 + (0.5 - 7.0) * ((lumi[lumi_index] - 59.83) / (3000.0 - 59.83))
                
                if(categ == 'CatC'):
                        frame_max = 7.0 + (2.5 - 7.0) * ((lumi[lumi_index] - 59.83) / (3000.0 - 59.83))
